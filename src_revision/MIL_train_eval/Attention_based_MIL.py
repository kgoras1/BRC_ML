...
def load_train_test_from_pkl(pkl_path):
    ...
    Returns:
        X_train (list), y_train (np.ndarray), X_test (list or None),
        y_test (np.ndarray or None), class_names (list or None),
        ids_train (list or None), ids_test (list or None)
...
# -------------------- Patient ID extraction (mirrors Datasplit_train_test.py) --------
...
# -------------------- Patient-level 80/10/10 split (binary OvR) --------------------
def patient_stratified_train_val_cal_split(bags, y_bin, slide_ids,
                                           val_size=0.10, cal_size=0.10, seed=42):
    """
    Patient-level 80/10/10 train/val/cal split for binary OvR labels.

    All slides from the same patient go into the same partition, preventing
    any form of patient-level data leakage between train, val, and cal.
    Stratification is by the dominant binary OvR label per patient.

    Falls back to slide-level split when patient count is too small for
    sklearn's stratified splitter (< 3 patients in either binary class).

    Args:
        bags (list): MIL bags (one per slide).
        y_bin (array-like): Binary labels in {0,1} for this OvR classifier.
        slide_ids (list or None): Slide identifiers aligned with bags/y_bin.
        val_size (float): Target validation fraction of total data (default 0.10).
        cal_size (float): Target calibration fraction of total data (default 0.10).
        seed (int): Random seed.

    Returns:
        X_train, X_val, X_cal (lists), y_train, y_val, y_cal (np.ndarray),
        n_patients_train, n_patients_val, n_patients_cal (int)
    """
    ...
    # ── 2. Need ≥ 3 of each class to create stratified 3-way split -----------
    if n_pos < 3 or n_neg < 3:
        X_tr, X_va, X_ca, y_tr, y_va, y_ca = _slide_level_split(bags, y_bin, val_size, cal_size, seed)
        return X_tr, X_va, X_ca, y_tr, y_va, y_ca, None, None, None

    # ── 3. First split off val patients ────────────────────────────────────────
    try:
    ...
    # ── 4. Split remaining patients into train and cal ─────────────────────────
    tc_labels = np.array([patient_label[p] for p in train_cal_pats])
    cal_fraction = cal_size / (1.0 - val_size)   # cal as fraction of train+cal pool
    try:
        train_pats, cal_pats = train_test_split(
            train_cal_pats, test_size=cal_fraction, random_state=seed, stratify=tc_labels
        )
    except ValueError:
        # If this split fails (e.g. very few cal patients), skip calibration
        train_pats = train_cal_pats
        cal_pats = []

    # ── 5. Map patients to bag indices ─────────────────────────────────────────
    ...
    y_val   = y_bin[np.array(val_idx,   dtype=int)]
    y_cal   = y_bin[np.array(cal_idx,   dtype=int)] if cal_idx else np.array([], dtype=int)

    # ── 6. Sanity: ensure val and cal have both classes (else drop cal) ─────────
    if len(np.unique(y_val)) < 2 or (len(y_cal) > 0 and len(np.unique(y_cal)) < 2):
        X_tr, X_va, X_ca, y_tr, y_va, y_ca = _slide_level_split(bags, y_bin, val_size, cal_size, seed)
        return X_tr, X_va, X_ca, y_tr, y_va, y_ca, None, None, None
...
def _slide_level_split(bags, y_bin, val_size=0.10, cal_size=0.10, seed=42):
    """
    Fallback slide-level 80/10/10 split when patient count is too small.
    Preserves class balance by splitting positives and negatives separately.
    """
    ...
    if n_pos < 2 or n_neg < 2:
        X_tr, X_va, y_tr, y_va = train_test_split(
            bags, y_bin, test_size=0.2, random_state=seed,
    ...
    if len(np.unique(y_val)) < 2 or len(np.unique(y_cal)) < 2:
        X_tr, X_va, y_tr, y_va = train_test_split(bags, y_bin, test_size=0.2, random_state=seed, stratify=y_bin)
        return X_tr, X_va, [], y_tr, y_va, np.array([], dtype=int)
...
def main():
    ...
    X_train_all, y_train_mc, X_test_all, y_test_mc, class_names_opt, ids_train_all, ids_test_all = \
    ...
    # Use TRAIN only for fitting and internal val/cal split
    bags = X_train_all
    labels_mc = np.asarray(y_train_mc, dtype=int)
    # slide IDs aligned with bags (needed for patient-level internal split)
    slide_ids_train = list(ids_train_all) if ids_train_all is not None else None
    if slide_ids_train is None:
        print("[WARN] No slide IDs found in PKL; internal val/cal split will be slide-level (not patient-level).")
    ...
    for k in range(n_classes):
        ...
        if args.debug:
            n_patients_total = len({get_patient_id(str(s)) for s in (slide_ids_train or [])})
            print(f"  [DEBUG] Overall TRAIN counts -> neg={total_neg}, pos={total_pos} "
                  f"(pos_rate={total_pos/(total_neg+total_pos):.3f}, unique patients={n_patients_total})")

        # 80/10/10: patient-level train/val/calib with stratification and safeguards
        X_train, X_val, X_cal, y_train, y_val, y_cal, n_pat_tr, n_pat_va, n_pat_ca = \
            patient_stratified_train_val_cal_split(
                bags, y_bin, slide_ids_train,
                val_size=args.val_size, cal_size=args.cal_size, seed=args.seed
            )
        ...
        if args.debug:
            ...
            pat_tr_str = f", patients={n_pat_tr}" if n_pat_tr is not None else ""
            pat_va_str = f", patients={n_pat_va}" if n_pat_va is not None else ""
            pat_ca_str = f", patients={n_pat_ca}" if n_pat_ca is not None else ""
            print(f"  [DEBUG] Val split counts:   neg={int(val_counts[0])}, pos={int(val_counts[1])} "
                  f"(pos_rate={(val_counts[1]/max(1, val_counts.sum())):.3f}{pat_va_str})")
            if calib_ds is None:
                print(f"  [DEBUG] Calib split counts: neg=0, pos=0 (no calib; T=1.0 will be used{pat_ca_str})")
            else:
                print(f"  [DEBUG] Calib split counts: neg={int(calib_counts[0])}, pos={int(calib_counts[1])} "
                      f"(pos_rate={(calib_counts[1]/max(1, calib_counts.sum())):.3f}{pat_ca_str})")

...
def bootstrap_case_ci(
    ...
    rng = np.random.default_rng(seed)
    n = len(case_true)
    metric_keys = ["accuracy", "balanced_accuracy", "f1", "precision", "recall", "auroc", "ap"]
    macro_keys  = ["f1", "precision", "recall", "balanced_accuracy", "auroc", "ap"]
...
def evaluate(
    ...
    LOGGER.info(f"Computing tile-level metrics (n_bootstrap={n_bootstrap}) ...")
    ...
    macro_ci_tile = bootstrap_macro_ci(y_test, tile_probs, n_classes, n_bootstrap=n_bootstrap)
    ...
    # Case-level plots and composite figure
    ...
    with open(txt_path, "w") as f:
        ...
        # ============================================================
        # PRIMARY: Case/patient-level (independent units)
        # CIs obtained by bootstrapping cases, not tiles or slides.
        # ============================================================
        f.write("=" * 70 + "\n")
        ...
        # ============================================================
        # SECONDARY: Slide-level (CIs bootstrapped over slides)
        # Treat as supplementary when patients have multiple slides.
        # ============================================================
        f.write("=" * 70 + "\n")
        ...
        # ============================================================
        # TERTIARY: Tile-level (supplementary — not independent units)
        # ============================================================
        f.write("=" * 70 + "\n")
        ...
        # ============================================================
        # AUXILIARY: source stratification, HER2, exports
        # ============================================================
        f.write("=" * 70 + "\n")
    ...
    results_json = {
        ...
        "slide": {
            "note": "SECONDARY/supplementary — bootstrapped by resampling slides",
            "per_class": _sanitize(per_class_slide),
            "averages": {
                "macro": avg_slide_macro,
                "macro_ci_bootstrap_slide": macro_ci_slide,
        ...
        "case": {
            **case_summary,
            "note": "PRIMARY results — per-class OvR metrics bootstrapped by resampling cases",
            "per_class": _sanitize(case_per_class),
